...
function makeStore(config) {
  ...
  let cache = null
  const recoverFrom = (target) => {
    try {
      if (!existsSync(target)) return null
      const v = JSON.parse(readFileSync(target, 'utf8'))
  ...
  const load = () => {
    ...
    let raw
    try { raw = readFileSync(file, 'utf8') } catch { cache = {}; return cache }
    try { cache = JSON.parse(raw) } catch {
      // 解析失败：先留证据，再尝试从 .bak 自动救回，最后才降级为空配置
      try { if (!existsSync(keep)) copyFileSync(file, keep) } catch { /* 备份失败不阻断 */ }
      cache = recoverFrom(keep) ?? {}
    }
    return cache
  }
  const loadRaw = () => {
    // 与 load 同源的自愈：文件读不出来时用 .bak 兜底，避免保存时用空配置覆盖旧密钥
    try {
      const v = JSON.parse(readFileSync(file, 'utf8'))
      if (v !== null && typeof v === 'object') return v
    } catch { /* 见下 */ }
    return recoverFrom(keep)
  }
  const save = (patch) => {
    const raw = loadRaw()
    ...
    try {
      ...
      if (existsSync(file)) {
        ...
        const nxt = JSON.stringify(next, null, 2)
        // 只在原文件非空、且内容确有变化时留一份前值备份（只留第一次）
        if (cur.trim() !== '' && cur !== nxt) {
          try { if (!existsSync(keep)) copyFileSync(file, keep) } catch { /* best effort */ }
        }
      }
      try {
        writeAtomic(file, JSON.stringify(next, null, 2))
      } catch {
        // 原子写失败（临时文件/改名被拒）时退回直写，保底不丢配置
        try { writeFileSync(file, JSON.stringify(next, null, 2), { encoding: 'utf8', mode: 0o600 }) } catch { /* best effort */ }
      }
    } catch { /* best effort */ }
    return next
  }
